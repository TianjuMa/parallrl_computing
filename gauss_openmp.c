...
/*   In the serial algorithm, there are three loops. We do the parallel at the second loop by
     consider the efficiency and feasibility and efficiency.
...
void gauss() {
  ...
  printf("Parallel programming using OpenMP.\n");

  /* Implicit gaussian elimination */
  int norm, row, col;

    // int norm, row, col;
  float multiplier;
  for (norm = 0; norm < N - 1; norm++) {
    // To make sure that do the parallel for second loop
    #pragma omp parallel private (multiplier, row, col) shared(A,B,norm) num_threads(procs) 
    {
